...
<head>
    ...
    <title>Confirm Transaction - {{ config('app.name') }}</title>
    <meta name="description" content="Confirm your mobile wallet transaction details">
        ...
        <div class="p-4">
            ...
            <nav class="mt-6">
                <ul class="space-y-1">
                    ...
                    <li>
                        <a href="{{ route('transactions.initiate') }}" class="flex items-center px-4 py-3 text-dark hover:bg-primary hover:text-white rounded-lg transition-colors {{ request()->routeIs('transactions.initiate') || request()->routeIs('transactions.confirm') ? 'bg-primary text-white' : '' }}">
    ...
    <main class="main-content min-h-screen bg-light pb-12">
        ...
        <header class="hidden lg:block bg-white shadow-sm sticky top-0 z-20">
            <div class="container mx-auto px-6 py-4 flex justify-between items-center">
                <h1 class="text-2xl font-bold text-dark">Confirm Transaction</h1>
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6">
                <div class="p-6">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                        <h2 class="text-xl font-bold text-dark mb-2 md:mb-0">Confirm Transaction Details</h2>
                        
                        <div class="flex space-x-2">
                            <a href="{{ route('transactions.initiate') }}" class="inline-flex items-center px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded text-sm text-gray-700 transition">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative mt-4">
                            <div class="overflow-hidden h-2 text-xs flex rounded bg-gray-200">
                                <div style="width: 66.6%" class="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-primary"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-light rounded-lg p-6 mb-6">
                        <div class="flex flex-col md:flex-row items-start md:items-center mb-6">
                            <div class="flex-shrink-0 mb-4 md:mb-0 md:mr-6">
                                @if($transaction->wallet_provider)
                                    @if($transaction->wallet_provider->api_code === 'airtel')
                                        <img class="h-16 w-16 rounded-full" src="{{ asset('assets/img/airtel.png') }}" alt="Airtel">
                                    @elseif($transaction->wallet_provider->api_code === 'mtn')
                                        <img class="h-16 w-16 rounded-full" src="{{ asset('assets/img/mtn.jpg') }}" alt="MTN">
                                    @elseif($transaction->wallet_provider->api_code === 'zamtel')
                                        <img class="h-16 w-16 rounded-full" src="{{ asset('assets/img/zamtel.jpg') }}" alt="Zamtel">
                                    @else
                                        <div class="h-16 w-16 rounded-full bg-primary text-white flex items-center justify-center text-2xl">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-dark">{{ $transaction->wallet_provider->name ?? 'Mobile Wallet' }} Transfer</h3>
                                <p class="text-gray-600">You are about to send money to {{ $transaction->recipient_name ?: 'a mobile wallet' }}</p>
                        ...
                        <div class="border-t border-gray-200 pt-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 mb-1">Transaction Amount</h4>
                                    <p class="text-xl font-bold text-dark">K{{ number_format($transaction->amount, 2) }}</p>
                                </div>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 mb-1">Fee (4%)</h4>
                                    <p class="text-xl font-bold text-dark">K{{ number_format($transaction->fee_amount, 2) }}</p>
                                </div>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 mb-1">Total Amount</h4>
                                    <p class="text-2xl font-bold text-primary">K{{ number_format($transaction->total_amount, 2) }}</p>
                                </div>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 mb-1">Transaction Reference</h4>
                                    <p class="text-lg font-medium text-dark">{{ $transaction->uuid }}</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="border-t border-gray-200 mt-4 pt-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    @if($transaction->purpose)
                                        <div>
                                            <h4 class="text-sm font-medium text-gray-500 mb-1">Purpose</h4>
                    ...
                    <div class="bg-primary bg-opacity-5 rounded-lg p-4 mb-6 border border-primary border-opacity-20">
                        <div class="flex items-start">
                            <div class="flex-shrink-0 mt-0.5">
                                <i class="fas fa-info-circle text-primary"></i>
                            </div>
                            <div class="ml-3">
                                <h3 class="text-sm font-medium text-dark">Important Information</h3>
                                <ul class="mt-1 text-sm text-gray-600 list-disc list-inside space-y-1">
                                    <li>Please verify all details before proceeding to payment</li>
                                    <li>Once payment is completed, funds will be sent to the mobile wallet immediately</li>
                                    <li>You will receive a confirmation SMS and email once the transaction is complete</li>
                    ...
                    <div class="flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-3">
                        <a href="{{ route('transactions.initiate') }}" class="flex-1 inline-flex justify-center items-center px-4 py-3 border border-gray-300 shadow-sm text-base font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                            <i class="fas fa-arrow-left mr-2"></i> Edit Details
                        </a>
                        
                        <form action="{{ route('transactions.process') }}" method="POST" class="flex-1">
                            @csrf
