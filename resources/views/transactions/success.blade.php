...
<aside id="sidebar" class="sidebar bg-white shadow-md h-screen fixed top-0 left-0 overflow-y-auto z-40">
            ...
            <ul class="space-y-1">
                <li>
                    <a href="{{ route('dashboard') }}"
                       class="flex items-center px-4 py-3 text-dark hover:bg-primary hover:text-white rounded-lg transition-colors {{ request()->routeIs('dashboard') ? 'bg-primary text-white' : '' }}">
                        <i class="fas fa-home w-6"></i>
                        <span>Dashboard</span>
                    </a>
                </li>
                <li>
                    <a href="{{ route('transactions.initiate') }}"
                       class="flex items-center px-4 py-3 text-dark hover:bg-primary hover:text-white rounded-lg transition-colors {{ request()->routeIs('transactions.initiate') || request()->routeIs('transactions.confirm') || request()->routeIs('transactions.payment') || request()->routeIs('transactions.success') ? 'bg-primary text-white' : '' }}">
                ...
                <li>
                    <a href="{{ route('transactions.history') }}"
                       class="flex items-center px-4 py-3 text-dark hover:bg-primary hover:text-white rounded-lg transition-colors {{ request()->routeIs('transactions.history') ? 'bg-primary text-white' : '' }}">
                ...
                <li>
                    <a href="{{ route('beneficiaries.index') }}"
                       class="flex items-center px-4 py-3 text-dark hover:bg-primary hover:text-white rounded-lg transition-colors {{ request()->routeIs('beneficiaries.index') ? 'bg-primary text-white' : '' }}">
                ...
                <li>
                    <a href="{{ route('profile.index') }}"
                       class="flex items-center px-4 py-3 text-dark hover:bg-primary hover:text-white rounded-lg transition-colors {{ request()->routeIs('profile.index') ? 'bg-primary text-white' : '' }}">
                ...
                <li>
                    <a href="{{ route('profile.kyc') }}"
                       class="flex items-center px-4 py-3 text-dark hover:bg-primary hover:text-white rounded-lg transition-colors {{ request()->routeIs('profile.kyc') ? 'bg-primary text-white' : '' }}">
                        <i class="fas fa-id-card w-6"></i>
                        <span>KYC Verification</span>
                        @if(auth()->user()->verification_level === 'basic')
                            <span
                                class="ml-auto inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-warning text-dark">
                                    Basic
                                </span>
                        @endif
                    </a>
                ...
                <li>
                    <a href="{{ route('support') }}"
                       class="flex items-center px-4 py-3 text-dark hover:bg-primary hover:text-white rounded-lg transition-colors {{ request()->routeIs('support') ? 'bg-primary text-white' : '' }}">
    ...
    <div class="p-4 mt-auto border-t">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                    class="flex items-center px-4 py-3 text-dark hover:bg-gray-100 rounded-lg transition-colors w-full">
    ...
    <div class="container mx-auto px-4 py-6">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6 text-center py-10">
                <div
                    class="w-24 h-24 mx-auto bg-success bg-opacity-10 rounded-full flex items-center justify-center mb-6">
                    <svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="25" cy="25" r="24" stroke="#28A745" stroke-width="2"/>
                        <path d="M15 25L22 32L35 18" stroke="#28A745" stroke-width="3" stroke-linecap="round"
                              stroke-linejoin="round" class="checkmark"/>
                    </svg>
                </div>

                <h2 class="text-2xl font-bold text-dark mb-2">Transaction Successful!</h2>
                <p class="text-gray-600 mb-6">Your payment has been processed and funds have been sent to the mobile
                    wallet.</p>

                <div class="flex flex-col md:flex-row justify-center space-y-3 md:space-y-0 md:space-x-3 px-6">
                    <a href="{{ route('transactions.show', $transaction->uuid) }}"
                       class="inline-flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                        <i class="fas fa-receipt mr-2"></i> View Receipt
                    </a>
                    <a href="{{ route('dashboard') }}"
                       class="inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
            ...
            <div class="receipt-container mb-6">
                <div class="flex justify-between items-start mb-6">
                    ...
                    <div class="text-right">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-success bg-opacity-10 text-success">
                                Completed
                            </span>
                ...
                <div class="border-t border-dashed border-gray-200 my-4 pt-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-dark font-medium">{{ $transaction->reference_2 }}</p>
                        </div>
                                ...
                                @if($transaction->wallet_provider)
                                    @if($transaction->wallet_provider->api_code === 'airtel')
                                        <img class="h-5 w-5 rounded-full mr-2"
                                             src="{{ asset('assets/img/airtel.png') }}" alt="Airtel">
                                    @elseif($transaction->wallet_provider->api_code === 'mtn')
                                        <img class="h-5 w-5 rounded-full mr-2" src="{{ asset('assets/img/mtn.jpg') }}"
                                             alt="MTN">
                                    @elseif($transaction->wallet_provider->api_code === 'zamtel')
                                        <img class="h-5 w-5 rounded-full mr-2"
                                             src="{{ asset('assets/img/zamtel.jpg') }}" alt="Zamtel">
                                    @endif
                                @endif
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden p-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-start">
                        <div
                            class="flex-shrink-0 h-6 w-6 rounded-full bg-primary bg-opacity-10 text-primary flex items-center justify-center mr-3">
                            <i class="fas fa-mobile-alt text-xs"></i>
                        </div>
                        <div>
                            <h4 class="text-sm font-medium text-dark">Check Your Mobile Wallet</h4>
                            <p class="text-sm text-gray-600">The funds should be available in the recipient's mobile
                    ...
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">We've sent a receipt to your email address for your
                                records.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">You can view this transaction and all your past
                                transactions in your transaction history.</p>
                        </div>
                    </div>
                </div>
